 ...
 * structure, footprint logic, fit framing). The dynamic component handles HOW each brand
 * is currently performing across the review-sample dataset.
 *
 * DATA SHAPE AND SQL SPEC:
 * See getBrandComparisonAggregates() in lib/data/reviews.ts for the canonical
 * client-side implementation. The equivalent SQL is:
 *
 *   SELECT
 *     CASE
 *       WHEN bucket = 'inkout' THEN 'inkOUT'
 *       WHEN provider_name ILIKE '%Removery%' THEN 'Removery'
 *     END AS brand,
 *     location_city AS city,
 *     COUNT(*) AS scrape_sample_size,
 *     ROUND(AVG(star_rating)::numeric, 2) AS scrape_avg_stars,
 ...
 *     SUM(CASE WHEN scarring_mentioned='Positive' THEN 1 ELSE 0 END) AS scarring_positive,
 *     SUM(CASE WHEN scarring_mentioned='Yes' THEN 1 ELSE 0 END) AS scarring_yes,
 ...
 *   WHERE (bucket = 'inkout' OR provider_name ILIKE '%Removery%')
 *     AND status = 'published'
 *     AND bucket IN ('inkout','competitor')
 *   GROUP BY brand, city
 *   ORDER BY brand, city;
 *
 * INKOUT BUCKET AGGREGATION:
 * inkOUT is identified by bucket='inkout'. tatt2away bucket is excluded per codebase
 * convention (applyPublicFilters "any" scope). Tampa inkOUT has _place_id=NULL but this
 * query groups by brand+city so the null does not matter.
 *
 * REMOVERY COVERAGE GAPS:
 * Removery rows exist for Austin and Chicago in the current scrape. Tampa (Westshore),
 * Houston (3 locations), and Round Rock are confirmed-real Removery markets not yet
 * captured. BrandComparisonEvidence renders them as "Review sample pending" via the
 * brandBPendingCities prop.
 *
 * NEW DATA NOTE:
 * Cross-city query surfaces an inkOUT location in Draper UT (HQ market). This is the
 * deepest inkOUT review sample in the dataset and is included in the evidence table.
 *
 * COMPONENT REUSABILITY:
 * BrandComparisonEvidence is parameterized by brandA and brandB strings and can be
 * reused for /comparisons/inkout-vs-laseraway, /comparisons/removery-vs-laseraway, etc.
 */
      ...
      <section className="bg-(--bg)">
        <Container>
          <div className="mx-auto max-w-2xl divide-y divide-(--line)">
            ...
            {/* At a glance */}
            <GuideSection heading="inkOUT vs Removery at a Glance">
            ...
            {/* TEPR vs PicoWay */}
            <GuideSection heading="The Key Difference: TEPR vs PicoWay">
              <p className="font-sans text-[15px] leading-relaxed text-(--muted)">
            ...
            {/* Pricing */}
            <GuideSection heading="inkOUT vs Removery: Pricing and Cost Comparison">
              <p className="font-sans text-[15px] leading-relaxed text-(--muted)">
            ...
            <GuideSection heading="inkOUT vs Removery: Results, Scarring, and Pain">
              <div className="space-y-3">
                {[
                  {
                    title: "Results",
            ...
            <GuideSection heading="inkOUT vs Removery: Best for Dark Skin, Color Ink, and Complete Removal">
              <p className="font-sans text-[15px] leading-relaxed text-(--muted)">
            ...
            {/* Verdict */}
            <GuideSection heading="Our Verdict: inkOUT or Removery?">
              <p className="font-sans text-[15px] leading-relaxed text-(--muted)">
