    ...
    <section aria-label="Treatment Guides" className="py-16">
      ...
      <Container>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          {guides.slice(0, 2).map((guide, i) => (
            <Link
              ...
              className="group relative flex aspect-3/2 w-full overflow-hidden rounded-xl shadow-md bg-card-dark"
            >
              <div className="absolute inset-0" style={{ zIndex: 0 }}>
                <Image src={assetPath(guide.image)} alt={guide.title} fill sizes="(max-width: 640px) 100vw, 50vw" className="object-cover object-center transition-transform duration-500 group-hover:scale-[1.03]" />
              </div>
              <div className="absolute inset-0 flex flex-col justify-between p-6" style={{ zIndex: 1, background: "linear-gradient(to top, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.35) 55%, rgba(0,0,0,0.08) 100%)" }}>
                {/* Pill badge */}
                ...
                {/* Text */}
                <div>
                  <h3 className="text-xl font-bold leading-snug tracking-wide uppercase" style={{ color: "#ffffff" }}>
                    {guide.title}
                  </h3>
                  <p className="mt-2 text-xs leading-relaxed line-clamp-2" style={{ color: "rgba(255,255,255,0.72)" }}>
                    {guide.description}
                  </p>
                  <p className="mt-3 inline-flex items-center gap-1 text-xs font-semibold" style={{ color: "rgba(255,255,255,0.9)" }}>
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {guides.slice(2).map((guide, i) => (
            <Link
              key={guide.slug}
              href={`/guides/${guide.slug}`}
              className="group relative flex aspect-4/3 w-full overflow-hidden rounded-xl shadow-md bg-card-dark"
            >
              <div className="absolute inset-0" style={{ zIndex: 0 }}>
                <Image src={assetPath(guide.image)} alt={guide.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover object-center transition-transform duration-500 group-hover:scale-[1.03]" />
              </div>
              <div className="absolute inset-0 flex flex-col justify-between p-5" style={{ zIndex: 1, background: "linear-gradient(to top, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.35) 55%, rgba(0,0,0,0.08) 100%)" }}>
                {/* Pill badge */}
                <div className="ml-auto rounded-full bg-black/60 px-3 py-1 backdrop-blur-sm">
                  <span className="text-xs font-semibold uppercase tracking-wider text-white/80">
                    {PILL_LABELS[guide.slug] ?? "WHAT TO EXPECT"}
                ...
                {/* Text */}
                <div>
                  <h3 className="text-base font-bold leading-snug tracking-wide uppercase" style={{ color: "#ffffff" }}>
                    {guide.title}
                  </h3>
                  <span className="sr-only">{guide.description}</span>
                  <p className="mt-2.5 inline-flex items-center gap-1 text-xs font-semibold" style={{ color: "rgba(255,255,255,0.9)" }}>
                    Read guide <ChevronRightIcon className="size-3.5" />
                  </p>
                </div>
              </div>
