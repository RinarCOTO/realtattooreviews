    ...
    <div className="relative grid gap-4 lg:grid-cols-2">
      ...
      <div className="relative z-1 border border-(--line) bg-white p-5 rounded-xl shadow-card">
        <div className="flex items-center gap-2 mb-4">
          <DotGreen />
          <span className="font-sans text-[14px] font-bold tracking-widest uppercase text-heading">What They Do Well</span>
        </div>
        {pros.length === 0 ? (
          <p className="text-[14px] text-heading">Not enough review data to identify consistent positives.</p>
        ) : (
          <ul>
            {pros.map((pro, i) => (
              <li key={pro} className="flex items-start gap-4 border-t border-(--line) py-3">
                ...
                <span className="text-[14px] leading-relaxed text-heading">{pro}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <div className="relative z-1 border border-(--line) bg-white p-5 rounded-xl shadow-card">
        <div className="flex items-center gap-2 mb-4">
          <DotRed />
          <span className="font-sans text-[14px] font-bold tracking-widest uppercase text-heading">Where Users Hesitate</span>
        </div>
        {cons.length === 0 ? (
          <p className="text-[14px] text-heading">No consistent negative signals in the current review dataset.</p>
        ) : (
          <ul>
            {cons.map((con, i) => (
              <li key={con} className="flex items-start gap-4 border-t border-(--line) py-3">
                ...
                <span className="text-[14px] leading-relaxed text-heading">{con}</span>
              </li>
            ))}
          </ul>
        )}
