...
export default function InfoCard({ label, body, link, linkHref, beforeBookingNote }: InfoCardProps) {
  return (
    ...
    <div className="rounded-xl border border-border bg-white p-6 transition-shadow hover:shadow-md">
      <p className="mb-3 text-[17px] font-bold text-(--ink)">{label}</p>
      <p className="text-[15px] leading-relaxed text-heading">{body}</p>
      ...
      <Link href={linkHref} className="mt-4 inline-flex items-center gap-1 text-[14px] font-medium text-(--accent) hover:underline">
